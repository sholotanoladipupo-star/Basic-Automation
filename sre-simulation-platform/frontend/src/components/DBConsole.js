...
function hasDownService(systemState) {
    if (!systemState)
        return false;
    return Object.values(systemState.services).some(s => s.status === 'down');
}
function hasDownDB(systemState) {
    if (!systemState)
        return false;
...
function executeQuery(sql, currentDb, systemState) {
    const q = sql.trim();
        ...
        switch (tableName) {
            case 'accounts': {
                ...
                if (up.includes('LIMIT')) {
                    ...
                    if (lm)
                        rows = rows.slice(0, parseInt(lm[1]));
                }
...
// ─── Component ────────────────────────────────────────────────────────────────
export default function DBConsole({ systemState }) {
    const [expandedDbs, setExpandedDbs] = useState(new Set(['payments_db']));
    ...
    return (_jsxs("div", { className: "flex h-full bg-[#0d1117] text-[#e6edf3] font-mono text-xs overflow-hidden", children: [_jsxs("aside", { className: "w-[200px] flex-shrink-0 bg-[#161b22] border-r border-[#30363d] flex flex-col overflow-hidden", children: [_jsx("div", { className: "px-3 py-2 border-b border-[#30363d] bg-[#0d1117]", children: _jsx("span", { className: "text-[10px] text-[#8b949e] uppercase tracking-widest font-bold", children: "Databases" }) }), _jsx("div", { className: "flex-1 overflow-y-auto py-1", children: Object.entries(DB_TREE).map(([db, tables]) => {
                            const isExpanded = expandedDbs.has(db);
                            const isActive = currentDb === db;
                            return (_jsxs("div", { children: [_jsxs("button", { onClick: () => { toggleDb(db); setCurrentDb(db); }, className: `w-full text-left px-3 py-1.5 flex items-center gap-1.5 hover:bg-[#21262d] transition-colors ${isActive ? 'text-[#58a6ff]' : 'text-[#c9d1d9]'}`, children: [_jsx("span", { className: "text-[9px] text-[#8b949e]", children: isExpanded ? '▼' : '▶' }), _jsx("span", { className: "text-[10px] truncate", children: db })] }), isExpanded && (_jsx("div", { className: "ml-3 border-l border-[#30363d]", children: tables.map(table => (_jsxs("button", { onClick: () => selectTable(db, table), className: `w-full text-left px-3 py-1 text-[10px] hover:bg-[#21262d] transition-colors flex items-center gap-1.5 ${selectedTable === table && currentDb === db ? 'text-[#3fb950] bg-[#0f2a1a]' : 'text-[#8b949e]'}`, children: [_jsx("span", { className: "text-[8px] opacity-60", children: "\u229E" }), _jsx("span", { className: "truncate", children: table })] }, table))) }))] }, db));
                        }) }), _jsx("div", { className: "px-3 py-2 border-t border-[#30363d] text-[9px] text-[#8b949e]", children: _jsx("span", { className: "text-[#58a6ff]", children: currentDb }) })] }), _jsxs("main", { className: "flex-1 flex flex-col min-w-0 overflow-hidden", children: [_jsxs("div", { className: "flex items-center gap-1.5 px-3 py-2 border-b border-[#30363d] bg-[#161b22] flex-wrap", children: [_jsx("span", { className: "text-[9px] text-[#8b949e] mr-1", children: "Quick:" }), QUICK_QUERIES.map(qq => (_jsx("button", { onClick: () => applyQuickQuery(qq.sql), className: "px-2 py-0.5 text-[9px] bg-[#21262d] border border-[#30363d] text-[#c9d1d9] rounded hover:bg-[#30363d] hover:text-[#58a6ff] transition-colors", children: qq.label }, qq.label)))] }), _jsxs("div", { className: "relative bg-[#0d1117] border-b border-[#30363d]", children: [_jsx("textarea", { ref: textareaRef, value: query, onChange: e => setQuery(e.target.value), rows: 5, spellCheck: false, className: "w-full bg-transparent text-[#e6edf3] text-xs font-mono p-3 resize-none outline-none leading-5 placeholder-[#484f58]", placeholder: "-- Enter SQL query here (Ctrl+Enter to run)", style: { caretColor: '#58a6ff' } }), _jsxs("div", { className: "flex items-center justify-between px-3 py-2 border-t border-[#30363d] bg-[#161b22]", children: [_jsx("span", { className: "text-[9px] text-[#484f58]", children: "Ctrl+Enter to run" }), _jsxs("button", { onClick: runQuery, disabled: isRunning, className: "flex items-center gap-1.5 px-3 py-1 bg-[#238636] hover:bg-[#2ea043] disabled:opacity-50 text-white text-[10px] font-bold rounded transition-colors", children: [isRunning ? (_jsx("span", { className: "inline-block w-2.5 h-2.5 border border-white border-t-transparent rounded-full animate-spin" })) : (_jsx("span", { children: "\u25B6" })), "Run Query"] })] })] }), _jsxs("div", { className: "flex-1 flex flex-col min-h-0 overflow-hidden", children: [_jsxs("div", { className: "flex border-b border-[#30363d] bg-[#161b22]", children: [['results', 'plan'].map(tab => (_jsx("button", { onClick: () => setActiveTab(tab), className: `px-4 py-2 text-[10px] font-bold transition-colors border-b-2 ${activeTab === tab ? 'text-[#58a6ff] border-[#58a6ff]' : 'text-[#8b949e] border-transparent hover:text-[#c9d1d9]'}`, children: tab === 'results' ? 'Results' : 'Query Plan' }, tab))), result && (_jsxs("div", { className: "ml-auto flex items-center px-3 gap-3 text-[9px] text-[#8b949e]", children: [!result.isError && (_jsxs("span", { children: [result.rowCount, " ", result.rowCount === 1 ? 'row' : 'rows'] })), _jsxs("span", { children: [result.execTimeMs, " ms"] })] }))] }), _jsxs("div", { className: "flex-1 overflow-auto", children: [!result && (_jsx("div", { className: "flex items-center justify-center h-32 text-[#484f58] text-[11px]", children: "No results yet \u2014 run a query above" })), result && activeTab === 'results' && (_jsx(_Fragment, { children: result.isError ? (_jsx("div", { className: "p-4 text-[#f85149] text-xs leading-relaxed whitespace-pre-wrap", children: result.errorMessage })) : result.rows.length === 0 ? (_jsx("div", { className: "p-4 text-[#8b949e] text-xs", children: "Empty set (0 rows)" })) : (_jsxs("table", { className: "w-full text-[10px] border-collapse min-w-max", children: [_jsx("thead", { children: _jsx("tr", { className: "bg-[#161b22] border-b border-[#30363d] sticky top-0 z-10", children: result.columns.map(col => (_jsx("th", { className: "px-3 py-2 text-left text-[#8b949e] font-bold whitespace-nowrap border-r border-[#30363d] last:border-r-0", children: col }, col))) }) }), _jsx("tbody", { children: result.rows.map((row, i) => (_jsx("tr", { className: `border-b border-[#21262d] hover:bg-[#21262d] transition-colors ${i % 2 === 0 ? 'bg-[#0d1117]' : 'bg-[#161b22]'}`, children: result.columns.map(col => {
                                                            const val = row[col];
                                                            ...
                                                            return (_jsx("td", { className: `px-3 py-1.5 whitespace-nowrap border-r border-[#21262d] last:border-r-0 ${colorClass}`, children: val === null || val === undefined ? _jsx("span", { className: "text-[#484f58] italic", children: "NULL" }) : String(val) }, col));
                                                        }) }, i))) })] })) })), result && activeTab === 'plan' && (_jsx("div", { className: "p-4", children: result.isError ? (_jsx("div", { className: "text-[#f85149] text-xs", children: result.errorMessage })) : (_jsx("pre", { className: "text-[11px] text-[#8b949e] leading-relaxed whitespace-pre", children: queryPlanText })) }))] })] })] }), _jsxs("aside", { className: "w-[240px] flex-shrink-0 bg-[#161b22] border-l border-[#30363d] flex flex-col overflow-hidden", children: [_jsxs("div", { className: "px-3 py-2 border-b border-[#30363d] bg-[#0d1117]", children: [_jsx("span", { className: "text-[10px] text-[#8b949e] uppercase tracking-widest font-bold", children: "Schema" }), selectedTable && (_jsx("div", { className: "mt-0.5 text-[10px] text-[#58a6ff] truncate", children: selectedTable }))] }), _jsx("div", { className: "flex-1 overflow-y-auto", children: !selectedTable ? (_jsx("div", { className: "p-3 text-[9px] text-[#484f58]", children: "Click a table to inspect its schema" })) : schemaColumns.length === 0 ? (_jsx("div", { className: "p-3 text-[9px] text-[#484f58]", children: "No schema defined" })) : (_jsx("div", { className: "divide-y divide-[#21262d]", children: schemaColumns.map((col, i) => (_jsxs("div", { className: "px-3 py-2 hover:bg-[#21262d] transition-colors", children: [_jsxs("div", { className: "flex items-center justify-between gap-1 mb-0.5", children: [_jsx("span", { className: "text-[10px] text-[#e6edf3] font-bold truncate", children: col.name }), col.extra.includes('PK') && (_jsx("span", { className: "text-[8px] px-1 py-0 bg-[#2a1e00] text-[#d29922] border border-[#d29922] rounded flex-shrink-0", children: "PK" })), col.extra.includes('FK') && !col.extra.includes('PK') && (_jsx("span", { className: "text-[8px] px-1 py-0 bg-[#0a1a2a] text-[#58a6ff] border border-[#58a6ff] rounded flex-shrink-0", children: "FK" }))] }), _jsx("div", { className: "text-[9px] text-[#3fb950]", children: col.type }), col.extra && (_jsx("div", { className: "text-[8px] text-[#484f58] leading-tight mt-0.5 break-words", children: col.extra }))] }, i))) })) }), _jsxs("div", { className: "px-3 py-2 border-t border-[#30363d] text-[9px] text-[#484f58] space-y-0.5", children: [_jsxs("div", { className: "flex justify-between", children: [_jsx("span", { children: "Host" }), _jsx("span", { className: "text-[#8b949e]", children: "mysql-primary:3306" })] }), _jsxs("div", { className: "flex justify-between", children: [_jsx("span", { children: "DB" }), _jsx("span", { className: "text-[#58a6ff]", children: currentDb })] }), _jsxs("div", { className: "flex justify-between", children: [_jsx("span", { children: "User" }), _jsx("span", { className: "text-[#8b949e]", children: "readonly" })] }), systemState && (_jsxs("div", { className: "flex justify-between pt-0.5", children: [_jsx("span", { children: "Incidents" }), _jsx("span", { className: systemState.active_incidents.length > 0 ? 'text-[#f85149]' : 'text-[#3fb950]', children: systemState.active_incidents.length > 0 ? `${systemState.active_incidents.length} active` : 'none' })] }))] })] })] }));
}
